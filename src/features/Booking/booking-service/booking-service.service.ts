...
export class BookingServiceService {
  constructor(
    ...
    private readonly serviceModel: Model<AdminService>,
  ...
  private getOccupiedTimeSlots(
    ...
    const startIndex = TIME_SLOTS.findIndex(
      (slot) => slot.displayTime === startTimeSlot,
    );
    ...
    for (
      let i = 1;
      i < slotsNeeded && startIndex + i < TIME_SLOTS.length;
      i++
    ) {
  ...
  private async isEmployeeAvailableForDuration(
    ...
    return requiredSlots.every((slot) => {
      const slotAvailability = employee.availabilitySlots.find(
        (s) =>
          isSameDubaiDay(s.date, date) && s.timeSlot === slot && s.isAvailable,
      );
  ...
  async checkTimeSlotAvailability(
    ...
    if (serviceId && Types.ObjectId.isValid(serviceId)) {
      ...
      if (service && service.estimatedTime) {
        requiredSlots = this.getOccupiedTimeSlots(
          timeSlot,
    ...
    const availabilities = await Promise.all(
      requiredSlots.map(async (slot) => {
        const availableEmployees = await this.getAvailableEmployeesCount(
          date,
          slot,
        );
    ...
    const maxBookings = Math.max(
      ...availabilities.map((a) => a.currentBookings),
    );
    const minEmployees = Math.min(
      ...availabilities.map((a) => a.availableEmployees),
    );
  ...
  private async getAvailableEmployeesCount(
    date: Date,
    timeSlot: string,
  ): Promise<number> {
    ...
    console.log(
      `Checking availability for ${timeSlot} on ${date.toDateString()}`,
    );
    console.log(`Total employees in system: ${employees.length}`);

    if (employees.length === 0) {
      console.log(
        'No employees found in system - returning default capacity of 5',
      );
      return 5;
    }
    ...
    const availableCount = employees.filter((employee) => {
      const hasAvailabilitySlots =
        employee.availabilitySlots && employee.availabilitySlots.length > 0;

      if (!hasAvailabilitySlots) {
        console.log(
          `Employee ${employee._id} has no availability slots configured - treating as available`,
        );
        return true;
      }
  ...
  async createBooking(data: CreateBookingDto): Promise<Booking> {
    ...
    try {
      ...
      console.log(
        'Service found:',
        service ? `${service.name} (${service._id})` : 'null',
      );
      ...
      const requiredSlots = this.getOccupiedTimeSlots(
        data.timeSlot,
      ...
      for (const slot of requiredSlots) {
        ...
        const employeeBookingsCount = await this.bookingModel
          .countDocuments({
            assignedEmployeeId: employee._id,
            date: { $gte: dateStart, $lte: dateEnd },
            timeSlot: slot,
        ...
        if (employeeBookingsCount >= BOOKINGS_PER_EMPLOYEE) {
          const slotIndex = employee.availabilitySlots.findIndex(
            (s) => isSameDubaiDay(s.date, bookingDate) && s.timeSlot === slot,
          );
      ...
      try {
        ...
        const customerName =
          customer && 'fullName' in customer ? customer.fullName : 'Customer';
        ...
        const employeeUser = await this.userAuthService.getById(
  ...
  async getEmployeeBookings(employeeId: string): Promise<Booking[]> {
    ...
    const bookings = await this.bookingModel
      ...
      .populate(
        'addressId',
        'building apartment streetAddress area city emirate state zipCode landmark',
      )
  ...
  async ManagerUpdateBookingStatus(
    id: string,
  ...
  async EmployeeupdateBookingStatus(
    id: string,
    data: UpdateBookingDto,
  ): Promise<Booking | null> {
    const booking = await this.bookingModel
    ...
    // Prevent employees from starting the job before the scheduled time
    if (data.status === 'in-progress') {
      try {
        const timeSlot = booking.timeSlot; // e.g. '09:00'
        if (timeSlot && booking.date) {
          ...
          if (now < scheduled) {
            throw new BadRequestException(
              `Cannot start job before scheduled time (${timeSlot})`,
            );
          }
        }
      } catch (err) {
  ...
  async rescheduleBooking(
    ...
    booking.reminderNotified = false as any;
    booking.reminderSentAt = undefined as any;
    }
  ...
  async assignEmployee(
    ...
    if (!employee) {
      const userAuth = (await this.userAuthService.getById(
        employeeId,
      )) as UserAuth;

      if (!userAuth || userAuth.role !== 'Employee') {
        throw new NotFoundException(
          'Employee not found. The user may not have an employee profile or is not an employee.',
        );
      }
    ...
    if (!wasAlreadyAssigned) {
      try {
        ...
        console.error(
          'Error creating employee assignment notification:',
          notifError,
        );
      }
    }
  ...
  async addReview(bookingId: string, dto: CreateReviewDto) {
    ...
    if (!employeeId)
      throw new BadRequestException('No employee assigned to this booking');
  ...
  async resolveFlag(
    ...
    if (dto.resolved) {
      ...
      if (dto.resolvedBy)
        employee.flags[flagIndex].resolvedBy = new Types.ObjectId(
          dto.resolvedBy,
        );
    } else {
    ...
    return {
      message: `Flag #${flagIndex + 1} ${
        dto.resolved ? 'marked as resolved' : 'reopened'
      } successfully.`,
      employee,
    };
  }
